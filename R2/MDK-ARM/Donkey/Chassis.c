...
void GamePad_Velocity_Standard(void)
{
	...
	if (fabs(rocker_r) < 6)
		rocker_r = 0;
	float Rocker_GainT = GamePad_Velocity_Standard_AccelFlag?136:(GamePad_Velocity_Standard_SlowFlag? 97:77);
	float Rocker_GainR = GamePad_Velocity_Standard_AccelFlag?42: (GamePad_Velocity_Standard_SlowFlag? 35:23);
...
void GamePad_Velocity_Noheader(void)
{
	...
	rocker_r = -Game_Pad_Rocker_Data[2];
	// 陀螺仪逆时针为正
	float Rocker_GainT = GamePad_Velocity_Standard_AccelFlag?126:(GamePad_Velocity_Standard_SlowFlag?97:77);
	float Rocker_GainR = GamePad_Velocity_Standard_AccelFlag?42: (GamePad_Velocity_Standard_SlowFlag?35:23);
	...
	// Chassis_Velocity_Out(x*Rocker_GainT,y*Rocker_GainT,0);
...
void Mark_PID_Par_Init(void){
	...
	mark.brake_percent = Limit(0.08 + 0.000013 * mark.outlimit,0,0.7);
	mark.brake_gain = 0.13;
...
void Self_Lock_Auto(void){
	...
	// 跑点时候的自动自锁
	else if ((chassis.control_status == Self_Control))
	{
